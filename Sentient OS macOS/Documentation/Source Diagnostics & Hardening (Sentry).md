...
> **Note:** B1's whole in-place restore approach is **superseded by the planned B11** (stage-then-swap for update). Once B11 ships, update no longer mutates the live vault, so this `.bak` restore choreography is deleted — B1 remains the safety until then.
...
### B11 — PLANNED: unify KB build + update on stage-then-swap (supersedes B1/B2's approach)
**Not built yet — a design decision (2026-07-02) that replaces how B1 handled update.** Today **build** (`VaultGenerator.generate`) works in a throwaway staging dir and atomically swaps into the real vault on success (corruption-proof), while **update** (`VaultCloud.update`) edits the **live** vault **in place** with a `.bak` restore (the whole reason B1's restore choreography exists). Make **both** use the SAME pattern: **everything is written/edited in a temp staging folder first, then moved into the real vault only on success.**

- **Build:** unchanged in spirit — empty staging → codex writes → swap.
- **Update:** **seed staging with a copy of the current vault** (so codex has the existing notes for surgical edits), point `cwd` at staging, edit there, swap in on success. The real vault is then **never mutated mid-run**.
- **Deletes code:** B1's in-place `removeItem(vault)`→`copyItem(.bak→vault)`→restore-on-error dance in `update()` goes away entirely — a failure just discards the temp folder (real vault untouched). One staging-based flow instead of two.
- **Unified, durable resume (this also makes INITIAL fail-safe across a restart):** both modes carry `ResumeToken(sessionID, stagingPath)` **persisted to UserDefaults** (mirror `setMidEdit`; load in `init`, set on `usageLimit`, clear on success). Guard on load: only rehydrate if the staging dir still exists AND `sessionID != nil`, else start fresh. This folds in the separate "make `createResume` durable" fix — one mechanism for build + update.
- **Atomic swap:** use `FileManager.replaceItemAt` (a true same-volume atomic replace) for both, instead of build's current `removeItem`+`moveItem`+`.bak` juggle.
- **Orphan cleanup:** on a fresh (non-resume) run, sweep leftover `.sentientos-vault-staging-*` dirs before creating the new one (they currently leak).
- **⚠️ The one behavior change to design + doc — concurrent user edits.** This re-opens Arch §6's **[DECIDED] "update edits in place, no staging"** and its edit-sync seam (`VaultActivity.editorBusy`): a snapshot-then-swap would **clobber** any edit the user's (future) Knowledge editor makes to the real vault *during* a run. Fix: a **swap-time freshness check** — if the real vault changed since the staging copy was taken (mtime/hash), **don't swap**; discard staging and re-run next cycle (the user's edit wins; the cycle's notes are still in `CycleStore`). Moot until the editor is built, but must land with it. **Update Arch §6** when this ships (the "in place … no staging" line becomes "stage + atomic swap for both").
- **Net:** simpler (one flow, B1 dance removed), safer (real vault never mutated mid-run), and initial + iterative share one durable-resume mechanism.
...
- **B11 — KB stage-then-swap unification** (§6/B11). ⏳ **Planned (2026-07-02), not built.** Its own PR: unify build + update on stage-then-swap, delete B1's in-place restore, one durable `(sessionID, stagingPath)` resume for both (this is also the "make initial fail-safe" fix), atomic `replaceItemAt`, orphan-staging sweep, + the swap-time freshness check for the future editor. Touches Arch §6's [DECIDED] "in place" — update that doc when it ships.
