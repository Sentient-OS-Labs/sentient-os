...
**NOT yet exercised end-to-end** — a real `codex exec` build/update with an actual usage-limit resume (needs a live, expensive codex run). The file mechanics (the corruption-risk surface) are proven; the codex integration is build-verified + reviewed.
