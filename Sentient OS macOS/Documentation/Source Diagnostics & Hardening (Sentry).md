...
> **Real-run verified (2026-07-02, headless self-test on real hardware):** ✅ **B8** — `FilesSource.eligibleFiles` kept **150/150** files in one dir (the 300 cap is live, not the old 100). ✅ **B11** (which supersedes B1) — real codex `create` (xhigh, ~75s → 7 notes/5 folders) + `update` (high) against a scratch vault: staging → **atomic swap** → vault intact, README present, no notes lost, zero orphan staging. **Not exercisable on demand:** a forced codex usage-limit mid-run (depends on live quota) and B5's deadman (needs the root daemon install + password).

### B1 — VaultCloud restore can destroy the knowledge base (CRITICAL, data loss) — ✅ FIXED (2026-07-01, unmerged)
> **Note:** B1's whole in-place restore approach is **superseded by B11** (✅ built — stage-then-swap for update). Update no longer mutates the live vault, so this `.bak` restore choreography has been **deleted**. The description below is the historical B1 fix; B11 is the current behavior.
...
### B6 — AC / thermal / low-power gates — ✅ BUILT (2026-07-02)
...
**Fix applied:** corrected the `init` default `perDirectoryCap` 100 → **300** (single source of truth; `FileRoot.source` is the only caller and relies on the default). Also fixed three stale "100/dir" comments (file header, `eligibleFiles` doc, `cappedNewestFirst` doc). **✅ Real-run verified (2026-07-02):** `eligibleFiles` over 150 files in one dir kept all 150 (would've been 100 before). **Co-suspects NOT changed** (no evidence they misfire; review only if the over-skip persists): the **depth cap at 3 folder levels** (~243, `maxWalkDepth = 3`) and the **code-density heuristic** (~133) on mixed data+doc folders. Add counting (§7.8) so the next such regression is visible.
...
### B10 — IterativeRun's failure catch conflates extraction vs inference — ✅ FIXED (2026-07-02, with the extraction-rate sensor §7.8)
**Fix applied:** `attempt()` split into two catches → `AttemptResult{ok/extractionFailed/generateFailed}`. Extraction failures no longer touch the reload path (a corrupt file can't waste engine reloads anymore) and feed `SourceHealth`'s rolling extraction-rate → `files.extraction_degraded`. See the P3 phasing note.
...
### B11 — unify KB build + update on stage-then-swap (supersedes B1/B2's approach) — ✅ BUILT (2026-07-02, unmerged; mechanics-verified)
Both **build** (`VaultGenerator.generate`) and **update** (`VaultCloud.update`) now use ONE pattern: everything is written/edited in a temp staging folder, then swapped into the real vault only on success. The live vault is **never mutated mid-run** in either mode.

**Shipped:**
- **Shared helpers on `VaultGenerator`:** `newStagingDir(seedFrom:)` (build → empty; update → seeded with a copy of the vault), `sweepOrphanStaging(keeping:)`, `swapStagingIntoVault` (atomic same-volume `FileManager.replaceItemAt`; on any throw the vault is left intact), and `runCodexInStaging` (poller + usage-limit→`ResumeToken` mapping, shared by both).
- **Build:** `generate()` uses the helpers; kept its shape. **Update:** rewritten to seed staging with a vault copy, `cwd` = staging, edit there, atomic swap on success. **B1's in-place `removeItem`→`copyItem(.bak)`→restore dance is DELETED** — a non-fatal failure just discards the staging copy (live vault untouched); B1's `.bak`/`backupURL`/`setMidEdit` are gone.
- **Durable resume for BOTH (also the "make initial fail-safe" fix):** `createResume` + `updateResume` are now `ResumeToken(sessionID, stagingPath)` persisted to UserDefaults (`vault.create.resume` / `vault.update.resume`), loaded in `init`, cleared on success. `loadResume` discards a token whose staging dir is gone or whose `sessionID` is nil. `ResumeToken` is now `Codable`.
- **Orphan sweep:** `newStagingDir` sweeps leftover `.sentientos-vault-staging-*` dirs (siblings of the vault) before creating the new one.

**Verified (headless mechanics self-test, scratch vault, no codex):** empty build-staging; update seed is an independent copy; **the live vault has no changes mid-run**; atomic swap adds new + keeps existing + consumes staging; swap with no prior vault moves in; **a failed swap leaves the vault fully intact**; orphan sweep keeps only the resume dir; `ResumeToken` JSON round-trip. (One bug found + fixed by the test: `sweepOrphanStaging`'s `keeping` compare was trailing-slash-sensitive.)
**✅ Real-run verified end-to-end (2026-07-02):** a real `codex exec` `create` (xhigh, ~75s → 7 notes/5 folders) then `update` (high) against a scratch vault — staging → atomic swap → vault intact, README present, no notes lost, zero orphan staging. The only unexercised branch is a **forced usage-limit resume** (can't drain live quota on demand) — but its `ResumeToken` persistence + staging-reuse are unit-verified, and the happy-path codex+staging+swap is now proven under real conditions.

**✅ Swap-time freshness check — BUILT (2026-07-02)** now that the Knowledge editor (#93) writes note files directly into the live vault. Two guards in `update()`: (1) skip the whole merge if `VaultActivity.editorBusy` at start (don't waste a codex call while the user is actively editing); (2) capture `VaultGenerator.vaultFingerprint(vault)` (a SHA-256 over each `.md`'s `relpath|size|mtime`) at seed time — carried in the `ResumeToken` so it survives a usage-limit resume — and at swap time, if the live vault's fingerprint changed (the user saved a note mid-run), **abort the swap** (discard staging, emit `vault.update.stale_swap_averted`, re-run next cycle) rather than clobber their edit. Verified: the fingerprint is stable for an unchanged vault and changes on any note edit/add (headless self-test).

**⚠️ Arch §6 updated** (the old **[DECIDED] "update edits in place … no staging"** now reads "stage + atomic swap for both").
...
- **SidekickHotkeyMonitor:** `hotkey.tap_create_failed` **event** (whole voice path dead, no user signal — add a consecutive-failure counter across health ticks); breadcrumbs `hotkey.missed_release_reconciled`, `hotkey.max_hold_forced`. Tag OS major (permission behavior differs 15 vs 26) and the active key (right ⌘ / right ⌥ — an enum, never free text).
...
- **P1 — infra** (§4). ✅ **Built (2026-07-01, unmerged; isolated Debug build green):** `captureEvent` + `DiagLevel` (§4.1) · the **opt-out gate** `diagnosticsEnabled` (default ON) + **Release-only boot** + `startForDevTest()` + `applyEnabledChange()` (§4.5) · the **anonymous install-id** (§4.11) · the **SettingsView opt-out toggle** (§9) · `beforeSend`/`beforeBreadcrumb` **scrubber** (§4.6) · the **OS/app-version fingerprint stamp** auto-added to every event (§4.7) · the **1-hour cap** (§5, wired into IterativeRun, emits `source.hit_time_cap`). ⏳ **Deferred within P1→P2/P3 (no consumer yet — build with the sensors that use them):** `SourceFault` (§4.2), `RunDiagnostics` (§4.3), `SourceHealth` (§4.4), the run-end aggregated event + catch-all net (§4.8–4.9), the **DB `schema_fp` fingerprint** (§4.7, needs the DB sources' listing). **B7 PII source-scrub:** ✅ **done (2026-07-02)** — content-bearing Proactive/Command/Executor logs are now `#if DEBUG` (never a Release breadcrumb) or reduced to lengths; the scrubber remains the backstop for structured PII. See §6/B7.
- **P2 — the 5 highest-value sensors.** ✅ **Built (2026-07-01, unmerged; isolated Debug build green):** the **3 brittle-decoder tripwires** via a new `SourceHealth` store (§4.4) — WhatsApp zero-sessions + `no_opted_in_chats` + listing-collapse (§7.3), iMessage decode-rate collapse + listing-collapse (§7.4; threshold set at <50% not the doc's 90% to stay quiet on attachment-heavy accounts — see the inline note), Notes decode-rate collapse + undownloaded/decode-fail split + listing-collapse (§7.5). Each computes its counts **inline in the connector's own listing method and emits directly** (no `RunDiagnostics` singleton needed for these — that's only required for the deep per-stage breakdowns / the run-end aggregate, deferred to P3). Plus ✅ the **CodexCLI chokepoint** (§7.9): a new `Invocation.feature` tag set at all ~10 call sites (gmail / calendar / vault / proactive / gmail-write / calendar-write / giftletter / computer), and one `emitCodexFailure` seam wrapping `run()` + `runAgentCommand()` that emits `codex.failure` / `codex.agent_command` keyed by the CLIError **case name only** (never `.message`/stderr/prompt) + feature + model/effort/duration. Plus ✅ the **FDA probe** (§7.22): `Permissions.fdaProbeDetail()` now exposes `(granted, which_probe, errno)` (true-denial vs Terminal-attribution vs missing-file), and `reportProbe()` emits `fda.probe` when not cleanly granted — wired once per run in `IterativeRun` when a DB source is present (the top empty-morning signal). ⏳ **Remaining P2:** the full **`OvernightReport`** (§7.20) — the ~20-field empty-morning bundle + `runID` scheduler↔helper correlation. Deferred: it's a large assembly best built *with* the production-scheduler wiring (Arch §9) and validated on real overnight hardware; the FDA probe already covers the single highest-value cause.
- **P3 — the rest.** ✅ **Built (2026-07-01, unmerged; isolated Debug build green):** the "silent whole-run/source failure" events in IterativeRun/Engine — `engine.load_failed` (§7.1/F1, §7.12), `source.dropped` (F2, the FDA-denied/DB-copy-failed whole-source drop), `engine.hard_stop` (F9, the GPU-wedge cascade give-up with reload counts) · **Triage discriminator** (§7.13) — `Outcome.reason` splits garbled-reply from genuine junk, `triage.parse_failure_spike` on a real sample · **Files per-root yield collapse** (§7.8 core) — `files.listing_collapsed` per root, catches the B8-class over-skip regression · **mirror push-failed** (§7.18) — `mirror.push_failed` (HTTP status only) in the swallow-and-retry-forever path. **✅ Built 2026-07-02:** the **SQLiteDB `db.schema_error`** (§7.7) — emitted on any `prepare` failure (our SQL is static → a failure means a column rename), the uniform "Apple/Meta changed the schema" tripwire across all 4 DB sources, carrying the allowlisted `sqlite3_errmsg`; also fixed the silent `-wal` copy swallow → breadcrumb. Plus the **Gmail/Calendar shape-mismatch split** (§7.10/§7.11) — `gmail.parse.shape_mismatch` / `calendar.parse.shape_mismatch` fire when the JSON won't parse or the required `notable` key is absent (a codex/schema regression), while a genuine quiet week (`notable:false`) stays silent — so a broken cloud leg stops hiding as "nothing to report". **✅ Built 2026-07-02 (contained captures):** **Notify** (§7.23) `notify.not_authorized` (auth status only) + `notify.add_failed` — a denied permission silently killing proactive reminders is now visible; **AddressBook** (§7.6) `addressbook.no_store` for the `v22→v23` file-rename (column renames are already caught by `db.schema_error`); **CodexSetup** (§7.24) `codex_setup.step_failed {step, error, binary_found}` at each of the 3 steps (install/login/computerUse), error TYPE only. **✅ Built 2026-07-02: the executor scoreboard (§7.19) — the flagship "AI that DOES things" health metric.** A new `ExecutorScoreboard.record(method:source:outcome:duration:)` sink emits `executor.fire {method, source, outcome, duration_s, status_present, error_class}`, fed from BOTH `ProactiveExecutor.fire()` (source `proactive_card`) and `CommandRunModel.complete()` (source `voice`/`promptBar`). Outcomes: `fired` / `notFireable` / `failed` / `refused`. **Product-prompt change (signed off):** the 3 executor wrappers now require a final `STATUS: DONE — …` / `STATUS: COULD_NOT — …` sentinel, and the brittle `hasPrefix("COULD NOT")` success test is replaced by parsing it (legacy prefix still recognized as a fallback); a missing sentinel → optimistic `fired` but `status_present:false`, which measures the false-success RISK. **⚠️ `fired` = codex CLAIMED done, NOT verified** (labeled as such). **✅ Built 2026-07-02: B10 + the Files extraction-rate sensor (§7.8/§7.12).** `IterativeRun.attempt()` now splits EXTRACTION (`connector.load` — a corrupt/huge file) from GENERATE (`engine.generate` — the GPU wedge) into two catches returning `AttemptResult{ok/extractionFailed/generateFailed}`. An extraction failure no longer counts toward `consecutiveFailures` or triggers an engine reload (a real fix — corrupt files used to waste reloads); it's skipped, counted (`p.extractionFailed`), and fed to a rolling window in `SourceHealth` (`recordExtraction`, epoch-hour-bucketed, pruned to a week). At run-end `checkExtractionRate()` emits `files.extraction_degraded` if the rolling success rate < 50% with ≥30 samples — the `.pdf`/`.doc`-decoder-rot tripwire. Rolling math verified by self-test. ⏳ **Deferred (larger / riskier / lower-value-with-overlap):** the full Files skip-histogram with `whitelisted_lost`, **voice/computer-use internals** (§7.21 — large, least-observable), **ComputerUseSetup's richer DMG per-step trail** (§7.24 — reverse-engineered layout), `model.not_found` (§7.14 — overlaps `engine.load_failed`'s `model_present`), and `RunDiagnostics`/B10/run-end aggregate. Skipped by design: vault/generator *success* events (§1 is failure diagnostics, not metrics).

- **B11 — KB stage-then-swap unification** (§6/B11). ✅ **Built (2026-07-02, unmerged; mechanics-verified via headless self-test).** Unified build + update on stage-then-swap, deleted B1's in-place restore, one durable `(sessionID, stagingPath)` resume for both (this is also the "make initial fail-safe" fix), atomic `replaceItemAt`, orphan-staging sweep. Arch §6 updated. ✅ **Swap-time freshness check now BUILT** (2026-07-02) — the Knowledge editor (#93) writes the live vault, so `update()` skips while `editorBusy` and aborts the swap if the vault's fingerprint changed mid-run (never clobbers a user edit). **Not yet run end-to-end** with a real codex usage-limit resume.
