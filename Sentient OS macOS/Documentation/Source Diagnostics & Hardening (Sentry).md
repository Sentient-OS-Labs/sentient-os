...
- **P1 — infra** (§4): `captureEvent` + `SourceFault` + `RunDiagnostics` + `SourceHealth` + `diagnosticsEnabled` opt-in + SettingsView toggle + `beforeSend`/`beforeBreadcrumb` scrubber + fingerprint stamp + the catch-all net. Plus the **1-hour cap** (§5). Plus B7's PII scrub.
