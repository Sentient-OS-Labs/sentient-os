...
**Deferred (as planned): the swap-time freshness check** for concurrent edits from the future Knowledge editor (a `TODO(editor)` marker sits at the swap in `update()`). Moot today — the update is the only writer (single writer, serialized by the `VaultCloud` actor). Must land with the editor.
