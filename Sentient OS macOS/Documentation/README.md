...
| `Engine/` | On-device inference: `Engine` (LiteRT-LM/Gemma wrapper), `Triage` (the bouncer prompts + verdict parse), `ModelLocator` | `LiteRT-LM (On-device Inference Engine)/` |
| `Ingestion/` | The reading pipeline: `Connector` protocol + `ItemKey` + `IterativeRun` + `CycleStore` (crash-safe core), the per-source connectors (`Connectors/`), the cloud sources (`GmailConnect`, `CalendarConnect`), `PipelineActivity`, `FactoryReset` | `Iterative Core (Connectors).md` · `Gmail Connector (Codex).md` · `Calendar Connector (Codex).md` |
| `Sources/` | The raw source readers: `FilesSource`, `WhatsAppSource`, `iMessageSource`, `NotesSource`, `ChatWindowing`, `AddressBookNames`, `SQLiteDB` (WAL-safe copy), `SourceSelection`/`CustomRoots`, the `Candidate`/`Artifact` value types | `Files Source (Skipping & Caps).md` · `WhatsApp Source (ChatStorage).md` · `iMessage Source (chat.db).md` · `Apple Notes Source (NoteStore).md` |
...
| `Cloud/` | The codex compute spine: `CodexCLI` (`codex exec` wrapper — the ONLY cloud-model path), `CodexSetup` + `ComputerUseSetup` (the 3-step setup engine), `MirrorClient` (the hosted MCP mirror) | `CodexCLI (codex exec Compute Spine).md` · `Codex Setup Handoff (Onboarding).md` · `Computer-Use Bootstrap (Codex Reverse-Engineering).md` · `Computer-Use Skill Patch (Confirmation Policy).md` · `MCP Mirror Client.md` |
| `Scheduling/` | The overnight 3am machinery: `OvernightScheduler`, the root `WakeHelper` (+ client/installer/protocol), `PowerState` gates, `LoginItem` | `Overnight Scheduler (Production Wiring & 18h Auto-Enable).md` |
| `Notch Magic/` | Sidekick: the right-⌘ hold-to-talk hotkey, on-device voice, the one shared command run, and the living notch overlay | `Notch Magic/Notch Magic.md` |
| `Store/` | Small UserDefaults-backed state: `LifetimeStats`, `SourceHealth` (diagnostics sensors), `ExecutorScoreboard`, the `Verdict` enum | — |
| `Diagnostics/` | `Log()` (the print replacement + dev-log tee), `CrashReporting` (Sentry), `Analytics` (TelemetryDeck) | `Crash Reporting (Sentry).md` · `Diagnostics (Sentry).md` · `Product Analytics (TelemetryDeck).md` · `Source Diagnostics & Hardening (Sentry).md` |
